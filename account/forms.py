...
from library.checks import check_phone_options, check_names
...
class SettingsForm(forms.ModelForm):
    ...
    class Meta:
        model = Account
        fields = ('username', 'first_name', 'last_name', 'bio', 'agent_image',
            'phone_number', 'email', 'alt_phone_number', 'linkd', 'fb',
            'insta', 'twit', 'snap', 'pin', 'goog', 'utube', 'mtup',)
    ...
    def clean_first_name(self):
        ...
        if not check_names.search(value):
            raise forms.ValidationError(
                _('Enter a valid first name. This value must contain only letters.')
                )
        return value.capitalize()
